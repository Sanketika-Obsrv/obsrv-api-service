...
import { apiId, errorCode } from "../../../controllers/DatasetStatus/DatasetStatus";
...
describe("DATASET STATUS API", () => {
    ...
    it("Dataset status failure: Invalid request payload provided", (done) => {
        chai
            ...
            .send(TestInputsForDatasetStatus.INVALID_SCHEMA)
            .end((err, res) => {
                ...
                res.body.error.code.should.be.eq("DATASET_STATUS_INVALID_INPUT")
                done();
            });
    });

    it("Dataset status failure: Connection to the database failed", (done) => {
